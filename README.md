...
当前阿里云单机构技术内测环境已通过固定公网 IPv4 + Let's Encrypt 短期 IP 证书运行，不强制购买域名；云端 API `0.2.0-beta.5` 已由提交 `544fbea` 部署，Alembic 已升级至 `20260823_0006`，Windows 客户端 `0.2.0-beta.7` 已通过签名更新服务发布。发布分支已同步至 GitHub。该环境仍未完成服务器扩容、Windows Authenticode、独立异地备份、机构真实 AI 调用和 50 人云端写入压测，不得作为生产安全环境或直接按 50 人规模开放。详见 [项目状态](docs/PROJECT_STATUS.md) 与 [云端部署手册](docs/CLOUD_DEPLOYMENT.md)。

当前发布提交已将教师 PPT 全屏播放、学生本机附件提交、课程级文件类型/大小限制、异步 AI 任务、学习 Agent、即梦 Seedance 视频、本地或私有 OpenAI 兼容模型连接和插件安装规范归入客户端 `0.2.0-beta.7`、API `0.2.0-beta.5`。API、数据库迁移和 Windows 更新产物均已部署；旧客户端可通过 `/desktop-updates/latest.json` 获取 `beta.7` 更新。
