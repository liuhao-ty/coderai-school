...
- 当前工作区已增加作品分类和工程包“另存为”：作品库区分工程包保存内容与 AI 生成内容，普通保存继续覆盖主工程包，另存副本不会产生新的作业提交版本。
- 当前工作区已修复 Agent 第二轮消息序号异常；本机附件提交可由学生、获授权教师和管理员通过提交版本接口预览或下载，教师批改列表增加按需加载的折叠历史。
- 学生生成图片即使自动审核通过也会先进入教师审批；审批前任务文件接口保持锁定，学生工作台显示等待状态，审批后可自动轮询或手动刷新预览。
- 新增 Alembic `20260826_0007` 及 SQLite 幂等迁移；该改动尚未提交、部署或进入 Windows 更新，当前云端仍为 API `0.2.0-beta.5` / Alembic `20260823_0006`。

## 当前试点部署

- 公网入口：`https://39.108.109.94`，机构代码 `coderai-pilot`，机构名称 `CoderAI`
- 数据方式：全新初始化；PostgreSQL、Redis、MinIO、FastAPI、Celery、Caddy 和监控容器均已运行
- 服务端发布：`0.2.0-beta.5`，构建标识 `544fbea61b8ddfce575a1c114dc16430887c19c7`；Alembic 已升级至 `20260823_0006`
...
2026-08-28 本轮必要及关联性检查：5 项后端定向用例通过，覆盖 Agent 连续两轮、学生/授课教师/管理员附件预览、同步及异步图片审批锁定与教师放行；前端生产构建通过，教师折叠历史与 Markdown 附件预览通过三个目标视口，学生图片待审批与放行刷新流程通过 Playwright。未运行全量门禁，也未执行桌面打包、Git 提交或云端部署。
